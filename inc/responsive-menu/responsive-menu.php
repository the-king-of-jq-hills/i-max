...
add_action( 'wp_enqueue_scripts', 'imax_enqueue_scripts' );

function imax_enqueue_scripts() {
...
function tx_search_form() {
	return '<form role="search" method="get" class="wpr-search-form" action="' . site_url() . '"><label><input type="search" class="wpr-search-field" placeholder="' . get_theme_mod('search_box_text', __(' Search...', 'i-max')) . '" value="" name="s" title="Search for:"></label></form>';
}

add_action('wp_footer', 'imax_menu', 100);
function imax_menu() {
		...
		<div id="wprmenu_bar" class="wprmenu_bar">
        	<div class="wprmenu-inner">
                ...
                <div class="menu_title">
                    <?php echo esc_attr(get_theme_mod('bar_title', __('MENU', 'i-max'))); ?>
                </div>
            </div>
		</div>
...
add_action('wp_head', 'imax_header_styles');